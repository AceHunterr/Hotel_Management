...
How many nights would you like to stay in for? > """)
            ...
            price = CalculatePrice(
                roomData, durationOfStay, category, numberOfGuests)
            print(
...
def CalculatePrice(roomData, durationOfStay, roomIndex, numberOfGuests):
    ...
    rooms_needed = (math.floor(int_rooms)+1) if (numberOfGuests %
                                                 capacity != 0) else int_rooms
    ...
    days = days_left % 7
    total_cost = ((months*per_month) + (weeks*per_week) +
                  (days*per_night)*rooms_needed)
